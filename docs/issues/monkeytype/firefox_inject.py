    ...
    for path in firefox_dir.iterdir():
        if path.is_dir() and ".default" in path.name:
            ...
            if new_storage.exists():
                profiles.append(
                    {"name": path.name, "path": path, "sqlite": new_storage, "storage_type": "new"}
                )
            elif old_storage.exists():
                profiles.append(
                    {"name": path.name, "path": path, "sqlite": old_storage, "storage_type": "old"}
                )
...
def inject_text_to_firefox(text: str, mode: str = "repeat", profile_name=None):
    ...
    try:
        ...
        if storage_type == "new":
            ...
            if existing:
                ...
                cursor.execute(
                    "UPDATE data SET value = ?, utf16_length = ? WHERE key = ?",
    ...
    print("✨ Opening Firefox to monkeytype.com...")
    print("=" * 60)
