...
class DictionaryConfig(BaseModel):
    ...
    enabled_languages: list[str] = Field(
        ...
        description="Language codes to load"
    )
    custom_paths: dict[str, str] = Field(
        default_factory=dict,
        description="Custom dictionary paths per language"
    )
    accept_all_mode: bool = Field(
        default=False,
        description="Skip validation, accept all 3+ letter words"
    )
    auto_fallback: bool = Field(
        default=True,
        description="Auto-enable accept_all if no dictionaries found"
    )
