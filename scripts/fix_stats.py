...
def main():
    ...
    cursor_local.execute("SELECT start_time, key_count, duration_ms, avg_wpm FROM bursts ORDER BY start_time")
    ...
    for keycode, key_name, percentage in key_distribution:
        ...
        cursor_local.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (keycode, key_name, "", 120, key_presses, 60, 200, int(datetime.now().timestamp() * 1000)))
    ...
    for date_str, date_bursts in sorted(bursts_by_date.items()):
        ...
        weighted_wpm = sum(b[2] * b[1] for b in date_bursts) / total_duration if total_duration > 0 else 0
        cursor_local.execute("""
    ...
    for keycode, key_name, percentage in key_distribution:
        ...
        """, (keycode, key_name, "", 120, key_presses, 60, 200, int(datetime.now().timestamp() * 1000), user.user_id))
    ...
    # Verify
    cursor_remote.execute("SELECT SUM(total_presses) FROM statistics WHERE user_id = %s", (user.user_id,))
    remote_stats = cursor_remote.fetchone()[0]
    cursor_remote.execute("SELECT SUM(total_keystrokes) FROM daily_summaries WHERE user_id = %s", (user.user_id,))
