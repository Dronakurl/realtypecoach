...
def compare_and_show(local_stats: dict, remote_stats: dict, user_id: str) -> None:
    ...
    if local_stats['bursts_time_range'][0]:
        local_start = datetime.fromtimestamp(local_stats['bursts_time_range'][0] / 1000).strftime('%Y-%m-%d')
        local_end = datetime.fromtimestamp(local_stats['bursts_time_range'][1] / 1000).strftime('%Y-%m-%d')
        print(f"   Range:  {local_start} to {local_end}")
    if remote_stats['bursts_time_range'][0]:
        remote_start = datetime.fromtimestamp(remote_stats['bursts_time_range'][0] / 1000).strftime('%Y-%m-%d')
        remote_end = datetime.fromtimestamp(remote_stats['bursts_time_range'][1] / 1000).strftime('%Y-%m-%d')
        print(f"   Range:  {remote_start} to {remote_end}")
    print()

    # Key statistics comparison
    print("📊 Key Statistics:")
    print(f"   Local:  {format_number(local_stats['stats_count'])} records, {local_stats['distinct_keys']} distinct keys")
    print(f"   Remote: {format_number(remote_stats['stats_count'])} records, {remote_stats['distinct_keys']} distinct keys")
    print()
    print(f"   Total key presses:")
    ...
    for i in range(min(5, len(local_stats['top_keys']), len(remote_stats['top_keys']))):
        ...
        print(f"      {i+1}. {local_key:15s}: Local={format_number(local_count):>12} | Remote={format_number(remote_count):>12}")
    print()

    # Word statistics comparison
    print("📊 Word Statistics:")
    print(f"   Local:  {format_number(local_stats['word_stats_count'])} records, {local_stats['distinct_words']} distinct words")
    print(f"   Remote: {format_number(remote_stats['word_stats_count'])} records, {remote_stats['distinct_words']} distinct words")
    print()
    ...
    for i in range(min(5, len(local_stats['top_words']), len(remote_stats['top_words']))):
        ...
        print(f"      {i+1}. {local_word:15s}: Local={format_number(local_count):>12} | Remote={format_number(remote_count):>12}")
    print()

    # Daily summaries comparison
    print("📊 Daily Summaries:")
    print(f"   Local:  {format_number(local_stats['daily_count'])} records, {local_stats['distinct_days']} days")
    print(f"   Remote: {format_number(remote_stats['daily_count'])} records, {remote_stats['distinct_days']} days")
    print()
    print(f"   Total keystrokes from daily summaries:")
    ...
    in_sync = (
        local_stats['bursts_count'] == remote_stats['bursts_count'] and
        local_stats['total_presses'] == remote_stats['total_presses'] and
        local_stats['total_observations'] == remote_stats['total_observations'] and
        local_stats['daily_keystrokes'] == remote_stats['daily_keystrokes']
    )
