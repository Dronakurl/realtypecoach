#!/usr/bin/env python3
"""Script to clean up mixed date formats in local SQLite database.

Converts all date fields to standardized ISO 8601 format (YYYY-MM-DD).
"""
...
# Import the proper crypto manager
...
def fix_date_formats():
    ...
    try:
        # Initialize crypto manager to handle encrypted database
        crypto = CryptoManager(db_path)
        encryption_key = crypto.get_key()
        
        if encryption_key is None:
            print("Error: Cannot access encryption key from keyring.")
            print("Please make sure you have unlocked your system keyring.")
            return False
        
        # Use the SQLite adapter which handles encryption properly
        adapter = SQLiteAdapter(db_path, encryption_key)
...
if __name__ == "__main__":
    print("=" * 60)
    print("RealTypeCoach Date Format Cleanup")
    print("=" * 60)
