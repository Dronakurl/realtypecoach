...
class TestCommonWordsGerman:
    ...
    def test_get_common_words_includes_german(self, temp_db, mock_wordfreq):
        ...
        mock_dict = MagicMock()
        mock_dict.get_loaded_languages.return_value = ["en", "de"]
        mock_dict.iter_top_n_words = MagicMock(
            side_effect=lambda lang, n, min_zipf: {
        ...
        with patch("core.storage.Dictionary", return_value=mock_dict):
            ...
            assert any(w in ["die", "der", "und"] for w in words), (
                f"Expected German words in {words}"
            )
    ...
    def test_slowest_words_common_only_includes_german(self, temp_db, mock_wordfreq):
        ...
        # Mock dictionary
        mock_dict = MagicMock()
        mock_dict.words = {"en": {"the", "be", "house"}, "de": {"der", "die", "haus", "katze"}}
        mock_dict.get_loaded_languages.return_value = ["en", "de"]

        def mock_get_word_best_zipf_frequency(word):
            word_lower = word.lower()
            # German words have high frequency
            if word_lower in ["der", "die", "haus", "katze"] or word_lower in [
                "the",
                "be",
                "house",
            ]:
        ...
        with patch("core.storage.Dictionary", return_value=mock_dict):
            ...
            result = storage.get_slowest_words_common_only(
                limit=10, layout="us", zipf_threshold=4.0
            )
