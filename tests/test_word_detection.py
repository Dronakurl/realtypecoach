...
class TestWordStorageWithDictionary:
    ...
    def test_valid_word_stored(self, storage_with_dict):
        ...
        with sqlite3.connect(storage_with_dict.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT word, backspace_count, editing_time_ms FROM word_statistics WHERE word = ?', ('hello',))
    ...
    def test_edited_word_with_backspace(self, storage_with_dict):
        ...
        with sqlite3.connect(storage_with_dict.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT word, backspace_count, editing_time_ms, total_duration_ms FROM word_statistics WHERE word = ?', ('shoes',))
            result = cursor.fetchone()
